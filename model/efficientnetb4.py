...
# #--------------------------------
# efficientnetb4_config = {
#     "num_classes": 2, # number of classes in your dataset
#     "inc": 3, # number of input channels (e.g., 3 for RGB images)
#     "dropout": False, # dropout rate
#     "mode": 'Original', # or 'adjust_channel'
#     "pretrained": False # or path to pretrained weights
# }
# if __name__ == '__main__':
#     model = EfficientNetB4(efficientnetb4_config)
#     torch.save(model.state_dict(),'/ssd6/Roy/LeGR_master_weihao/EfficientNetB4_org.pt')
...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        result = self.block(x)
        # print(f"After block, result shape: {result.shape}")
...
class EfficientNet(nn.Module):
    def __init__(self,
        ...
        for stage, args in enumerate(default_cnf):
            ...
            for i in range(round_repeats(cnf.pop(-1))):
                ...
                index = str(stage + 1) + chr(i + 97)  # stage1a, stage2a, stage2b, ...     修改一下，index长这样
                # 好好理一下 *cnf 操作
        ...
        layers.update({"top": ConvBNActivation(in_planes=last_conv_input_c,
                                               out_planes=last_conv_output_c, # b4: 1280 * 1.4 = 1792
                                               kernel_size=1,
                                               norm_layer=norm_layer)})
        ...
        # --------------------------------------------------------------------------------------------------------    
        classifier.append(nn.Linear(last_conv_output_c, num_classes)) #有先做avgpool 所以不用乘上(h,w)
        # classifier.append(nn.Linear(last_conv_output_c*8*8, num_classes))
        # --------------------------------------------------------------------------------------------------------
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        x = self.features(x)
        # print("After features:", x.shape)  # 打印形狀
        x = self.avgpool(x)
        # print("After avgpool:", x.shape)  # 打印形狀
        x = torch.flatten(x, 1)
        # print("After flatten:", x.shape)  # 打印形狀
        x = self.classifier(x)
        # print("After classifier:", x.shape)  # 打印形狀
        return x
...
if __name__ == "__main__":
    ...
    # torch.save(model.state_dict(),'/ssd6/Roy/LeGR_master_weihao/EfficientNetB4.pt')
